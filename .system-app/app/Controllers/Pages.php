...
class Pages extends BaseController {
    ...
    public function how_to_buy() {
        ...
        return view('Pages/how_to_buy', $data);
    }
    ...
    public function help() {
        ...
        if ($this->request->getPost('tombol')) {
            $data_post = [
                ...
                'pesan' => addslashes(trim(htmlspecialchars($this->request->getPost('pesan')))),
            ];

            if (empty($data_post['name']) OR empty($data_post['email']) OR empty($data_post['pesan'])) {
                $this->session->setFlashdata('error', 'Masih ada data yang kosong');
                return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
            } else {
                $this->M_Base->telegram($this->M_Base->u_get('chat_id'), "Pesan Bantuan\nNama : " . $data_post['name'] . "\nEmail : " . $data_post['email'] . "\nPesan : " . $data_post['pesan']);

                $this->session->setFlashdata('success', 'Pesan berhasil dikirim');
                return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
            }
        }
