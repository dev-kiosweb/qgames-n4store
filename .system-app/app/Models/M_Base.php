...
class M_Base extends Model {
	...
	public function email_invoice($to, $a, $b, $c, $d) {
		...
		$email->initialize([
			'SMTPHost' => $this->u_get('s_host'),
			'SMTPUser' => $this->u_get('s_user'),
			'SMTPPass' => $this->u_get('s_pass'),
			'SMTPPort' => $this->u_get('s_port'),
			'mailType' => 'html',
		]);
	...
	public function telegram($chat_id, $message_text) {
		$url = "https://api.telegram.org/bot1653320887:AAFacH3V_sQ-5O2e2hVHdhl42nJ267d5XRI/sendMessage?chat_id=".$chat_id."&text=" . urlencode($message_text);
	    $ch = curl_init();
	    $optArray = array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => true
